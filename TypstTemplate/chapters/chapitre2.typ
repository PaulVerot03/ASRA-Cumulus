...
Nous avons ensuite vidé le cache ARP de la Machine 1 et lancé un test de ping vers la Machine 2, la forceant à en envoyer un nouveau.
...
#strong[Explication du problème] : L'absence d'un protocole d'évitement de boucle provoque presque instantanément un effondrement du réseau.
\ \
...
Ce qui montre qu'un protocole de niveau 2 (STP/RSTP) est indispensable pour bloquer logiquement l'un des chemins et briser la boucle physique.
...
#sub[_NB_: Le protocole STP est activé par défaut sur Cumulus, cette étape peut être considérée comme superflue]
==== Rôle et état des ports
On peut voir l'état du protocole avec la commande ```bash net show bridge spanning-tree``` :
#grid(
  columns: 2,
  gutter: 3pt,
  figure(image("../figures/2/image6.png", width: 100%), caption: "Swicth 1"),
  figure(image("../figures/2/image8.png", width: 100%), caption: "Swicth 2"),
...
#list(
  [
    ...
    Le port swp3 du Switch 1 a été placé en rôle Alternate (Altn) et en état Discarding (disc). Il bloque le trafic pour casser la boucle, tout en restant à l'écoute d'éventuels changements de topologie
  ],
  [
...
== Test STP
Pour simuler une panne, on va desactiver un des lien actif (swp2/Switch 1). Le lien passif (swp3/Switch-2) devrait alors prendre le relais pour assurer la communication.\
