...
#figure(
  ...
  caption: [Vérification de l'état de l'agrégation],
)
...
Pour vérifier que le transfert continuerai même si un des liens tombait, j'ai créé un fichier de 1Gb avec la commande `fallocate -l 1G big.txt`, puis je l'ai copié dans le dossier `/mnt/backup_m1/`, après quelques instants, j'ai coupé l'interface swp3 sur le Switch-1. Ainsi, le transfert devrait passer sur l'autre lien. \
#figure(
  image("../figures/1/nfs-failover.png", width: 80%),
  caption: "sha256 des fichiers locaux (en bas) et distant (en haut)",
)
On peut constater sur le serveur que le fichier est bien apparut mais également que le checksum est identique, donc tout les bits ont correctement été transmis.
