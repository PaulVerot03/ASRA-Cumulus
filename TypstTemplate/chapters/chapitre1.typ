...
- Pour assurer l'isolation des machines, elles seront placées dans des VLANs différents et la communication entre ces réseaux sera interdite par des règles de pare-feu sur le Switch-3.
...
- #raw("net add hostname sw1")
  - Définit le nom d'hôte du switch.

- #raw("net add bridge bridge ports swp1,swp2,swp3,swp4")
  - Ajoute les interfaces physiques (`swp1` à `swp4`) au bridge principal.

- #raw("net add bridge bridge vlan-aware")
  - Active le mode "VLAN-aware" du bridge pour permettre la gestion des VLANs.

- #raw("net add interface swp2 bridge access 10")` \ `#raw("net add interface swp3,swp4 bridge access 20")
  - Configure les interfaces en mode accès:
    - `swp2` appartient au VLAN 10.
    - `swp3` et `swp4` appartiennent au VLAN 20.

- #raw("net add interface swp1 bridge trunk vlans 10,20")
  - Configure `swp1` en mode trunk pour transporter les VLANs 10 et 20.

- #raw("net add vlan 10 ip address 192.168.10.1/24") \ #raw("net add vlan 20 ip address 192.168.20.1/24") \ #raw("net add vlan 30 ip address 192.168.30.1/24")
...
- #raw("net commit")
  - Valide et applique toutes les modifications précédentes, analogue à un commit sur Git.== Configuration des Adresses IP sur les Machines
  \
=== Machine 1
Dans le fichier #raw("/etc/network/interfaces :")  
```bash 
auto ens36 
